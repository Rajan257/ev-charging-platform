...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      login(
        { id: userId, email, fullName: fullName || user?.fullName, role: role || user?.role },
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-dark-900">
      ...
      >
        ...
        <div className="glass-card p-8">
          ...
          {/* Demo credentials */}
          <div className="mt-5 p-3 rounded-xl bg-dark-700/50 border border-white/5">
            <p className="text-xs text-white/40 mb-2 font-medium">Demo credentials:</p>
            <button
              onClick={() => { setEmail('driver1@evroaminghub.in'); setPassword('password123') }}
              className="text-xs text-primary-400 hover:text-primary-300 block"
            >
              driver1@evroaminghub.in / password123
            </button>
            <button
              onClick={() => { setEmail('admin@evroaminghub.in'); setPassword('admin123') }}
              className="text-xs text-purple-400 hover:text-purple-300 block mt-1"
            >
              admin@evroaminghub.in / admin123
            </button>
          </div>
